...
from util.data import RndData, HdData, save_rnd_hd
from util.configcore import config
from util.decorators import exception_handler
...
def plot_MKM(
    ...
    filename = f"T-{tau_day}_{day}.png"
    ...
    # save bandwidths in file
    bandwidths = pd.DataFrame([RND.date, RND.tau_day, RND.h_m, RND.h_m2, RND.h_k]).T

    return fig_method, fig_strategy, bandwidths, filename
...
for day in days:
    ...
    for tau in taus:
        ...
        if (tau_day > tau_min) & (tau_day <= tau_max) & (tau["count"] > 25):
            fig_method, fig_strategy, bandwidths, filename = plot_MKM(
                RndData,
                HdData,
                day,
                tau_day,
            ...
            bandwidths.to_csv(
                config.app_config.data_rnd.joinpath("bandwidths.csv"),
                index=False,
                header=False,
                mode="a",
            )
